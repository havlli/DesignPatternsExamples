...
1.  [Database Connection Pooling](https://github.com/havlli/DesignPatternsExamples/blob/master/src/main/java/creational/objectpool/Database%20Connection%20Pooling.md): In web applications, database access is a common operation. A database connection pool can be created using the Object Pool design pattern to manage a pool of reusable database connections. Instead of creating a new database connection every time, the application can borrow a connection from the pool and return it when it is no longer needed.

2.  [Thread Pooling](https://github.com/havlli/DesignPatternsExamples/blob/master/src/main/java/creational/objectpool/Thread%20Pooling.md): In Java, the Executor Framework provides a way to manage threads and execute tasks asynchronously. An Object Pool can be used to manage a pool of threads to reduce the overhead of creating new threads and improve performance. Instead of creating a new thread every time a task is executed, the application can borrow a thread from the pool and return it when the task is completed.

3.  [Network Connection Pooling](https://github.com/havlli/DesignPatternsExamples/blob/master/src/main/java/creational/objectpool/Network%20Connection%20Pooling.md): In network-based applications, network connections are a common resource. An Object Pool can be used to manage a pool of reusable network connections. Instead of creating a new network connection every time, the application can borrow a connection from the pool and return it when it is no longer needed.
